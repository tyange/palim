...
- **단위 테스트(Vitest)**: 도메인 로직은 `core/`에 순수 TS로 모여 있어 단위 테스트로
  검증한다. `tokenizer.test.ts`(분류·자소·offset), `layout.test.ts`(칸 묶기·금칙·
  줄넘김·역산)가 있다. **도메인 규칙을 바꾸면 같은 PR에서 테스트도 갱신한다.**
  컴포넌트(GridEditor 등) 테스트는 아직 없다.
